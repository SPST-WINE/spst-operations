// FILE: lib/email/templates/waveAcceptedCarrier.ts
...
export function buildWaveAcceptedCarrierHtml(opts: {
  waveCode: string;
  waveId: string;
  ...
  const preheader = pickupDate
    ? `Wave accettata correttamente. Stato aggiornato: IN CORSO. Ritiro previsto ${pickupDate} ${pickupWindow}.`
    : `Wave accettata correttamente. Stato aggiornato: IN CORSO.`;

  const subject = `SPST • Wave accettata — ${waveCode}`;
            ...
            <tr>
              <td style="padding:24px;">
                ...
                <p style="margin:0 0 18px 0;color:#374151;font-size:14px;line-height:1.55;">
                  Puoi continuare a gestire i ritiri dalla pagina wave. Per urgenze operative, scrivici su
                  <a href="${escapeHtml(
                    waUrl
