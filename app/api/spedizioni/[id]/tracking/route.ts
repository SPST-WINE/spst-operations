...
export async function PATCH(
  ...
  const tracking_code = body?.tracking_code ?? body?.tracking ?? null;
  const carrier = body?.carrier ?? null;

  if (!tracking_code && !carrier) {
    return NextResponse.json({ ok: false, error: "MISSING_TRACKING_FIELDS" }, { status: 400 });
  }
  ...
  try {
    ...
    const { data, error } = await supaAdmin
      ...
      .eq("id", id)
      .select("id,carrier,tracking_code,status,updated_at")
      .single();
