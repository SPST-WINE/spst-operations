// app/dashboard/impostazioni/ImpostazioniClient.tsx
"use client";
...
export default function ImpostazioniClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!profileEmail) {
          setErrorMsg("Impossibile recuperare l'email del profilo. Contatta SPST.");
          setStatus("idle");
          return;
        }
        ...
        if (res.ok && body?.ok) {
          ...
          setErrorMsg(
            body?.message || body?.error || "Errore nel caricamento delle impostazioni."
          );
        }
      } catch (e: any) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!emailNorm) {
        setErrorMsg("Email non disponibile. Ricarica la pagina o contatta SPST.");
        setStatus("idle");
        return;
      }

      const res = await fetch(`/api/impostazioni?email=${encodeURIComponent(emailNorm)}`, {
        method: "POST",
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          {loading && (
            <span className="text-xs text-slate-400">Caricamento impostazioni…</span>
          )}
          {statusMsg && !errorMsg && (
